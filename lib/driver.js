...
module.exports = R.curry((server_url, Client) => {
  ...
  const DeleteIndexUrl      = Url.deleteIndex(server_url)
  ...
  return {
    ...
    , exists: (name) =>
        Client.head( DEFAULT_HEADERS, AdminUrl(name) )
  ...
  , del: R.curry((bucket, docid, rev) =>
      Client.delete( DEFAULT_HEADERS, BucketUrl(bucket, docid), { rev: rev })
    )

    // docs is a list of docs, with or without _id and _rev
  , bulk_upsert: R.curry((bucket, docs) =>
      Client.post( DEFAULT_HEADERS, BucketUrl(bucket, '_bulk_docs'), { docs } )
    )
  ...
  , deleteIndex: R.curry((bucket, index) =>
      Client.delete(
        DEFAULT_HEADERS
        , DeleteIndexUrl(bucket, index)
      )
    )

  , ensureFullCommit: R.curry((bucket) =>
      Client.ensureFullCommit(
        DEFAULT_HEADERS
        , TypeUrl(Url.Type.EnsureFullCommit, bucket)
      )
    )
